...
def server(server_id):
    ...
    servers = get_servers()
    return render_template('server.html', servers=servers, channels=channels, server=server, users=users,show_columns=True)
...
def get_messages_grouped_by_day(channel_id):
    messages_query = text("SELECT * FROM view_messages_by_day WHERE channel_id = :channel_id")
